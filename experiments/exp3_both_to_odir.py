...
def main():
    ...
    ds_aptos = APTOSDataset(config['dataset_paths']['aptos'], transform=train_transform)
    ds_messidor = MessidorDataset(config['dataset_paths']['messidor'], transform=train_transform)
    
    train_dataset = ConcatDataset([ds_aptos, ds_messidor])
    
    # Evaluate on ODIR
    val_dataset = ODIRDataset(config['dataset_paths']['odir'], transform=val_transform)
    ...
    # 3. Train Model
    # ==========================
    print("Starting Training on APTOS and Messidor...")
